...
export async function PUT(request, { params }) {
    ...
    try {
        ...
        // Scope update to the authenticated user's entries only
        const entry = await Entry.findOneAndUpdate(
            { _id: id, userId: session.user.id },
            body,
            { new: true, runValidators: true }
        );
        if (!entry) return NextResponse.json({ success: false, error: 'Entrada no encontrada' }, { status: 404 });
...
export async function DELETE(request, { params }) {
    ...
    try {
        await dbConnect();
        // Scope deletion to the authenticated user's entries only
        const result = await Entry.deleteOne({ _id: id, userId: session.user.id });
